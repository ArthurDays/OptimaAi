...
(() => {
  ...
  }; var v = "cookiePreferences", $ = "G-11RH8STBFW", u = class { constructor(e = {}) { this.config = { delay: 1500, ...e }, this.elements = { banner: i("cookie-banner"), modal: i("cookie-modal"), acceptBtn: i("cookie-accept"), rejectBtn: i("cookie-reject"), customizeBtn: i("cookie-customize"), saveBtn: i("cookie-save-preferences"), closeBtn: i("cookie-modal-close"), checkboxes: { analytics: i("cookie-analytics"), marketing: i("cookie-marketing") } }, this.init() } init() { let e = this.getPreferences(); e ? this.applyPreferences(e) : setTimeout(() => this.showBanner(), this.config.delay), this.attachEvents() } attachEvents() { let { acceptBtn: e, rejectBtn: t, customizeBtn: s, saveBtn: o, closeBtn: c, modal: a } = this.elements; r(e, "click", () => this.acceptAll()), r(t, "click", () => this.rejectAll()), r(s, "click", () => this.openModal()), r(o, "click", () => this.savePreferences()), r(c, "click", () => this.closeModal()), r(a, "click", l => { l.target === a && (this.closeModal(), this.getPreferences() || this.showBanner()) }) } getPreferences() { return h.get(v) } savePreferencesData(e) { let t = { ...e, timestamp: new Date().toISOString() }; h.set(v, t), this.applyPreferences(t) } applyPreferences(e) { window[`ga-disable-${$}`] = !e.analytics } acceptAll() { this.savePreferencesData({ essential: !0, analytics: !0, marketing: !0 }), this.hideBanner(), this.closeModal() } rejectAll() { this.savePreferencesData({ essential: !0, analytics: !1, marketing: !1 }), this.hideBanner(), this.closeModal() } savePreferences() { let { checkboxes: e } = this.elements; this.savePreferencesData({ essential: !0, analytics: e.analytics?.checked || !1, marketing: e.marketing?.checked || !1 }), this.closeModal() } openModal() { let e = this.getPreferences(), { checkboxes: t, modal: s } = this.elements; e && (t.analytics && (t.analytics.checked = e.analytics), t.marketing && (t.marketing.checked = e.marketing)), s?.classList.add("show"), document.body.style.overflow = "hidden", this.hideBanner() } closeModal() { this.elements.modal?.classList.remove("show"), document.body.style.overflow = "" } showBanner() { this.elements.banner?.classList.add("show") } hideBanner() { this.elements.banner?.classList.remove("show") } static checkConsent(e) { return h.get(v)?.[e] || !1 } }
    , G = u.checkConsent; var T = n => { let e = n.replace(/\D/g, "").slice(0, 11); return e.length > 6 ? `(${e.slice(0, 2)}) ${e.slice(2, 7)}-${e.slice(7)}` : e.length > 2 ? `(${e.slice(0, 2)}) ${e.slice(2)}` : e.length > 0 ? `(${e}` : "" }; var L = n => { const regex = /^[a-zA-Z0-9.!#$%&'*+\/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/; return regex.test(n) && !n.includes('..') && !n.endsWith('.') && !n.startsWith('.') }, I = n => { let e = n.replace(/\D/g, ""); return e.length >= 10 && e.length <= 11 }, S = (n, min = 3) => n?.trim().length >= min;
  var VNF = n => {
    const parts = n.trim().split(/\s+/);
    return parts.length >= 1 && parts.every(p => p.length >= 2);
  };
  var g = class {
    constructor() { this.init() } init() { this.theme = new p, this.cookies = new u, this.initMobileMenu(), this.initScrollAnimations(), this.initCounters(), this.initFAQ(), this.initScrollToTop(), this.initParallax(), this.initForm(), this.initPhoneMask(), this.initCTATracking(), this.refreshIcons(), this.retryPendingForms() } initMobileMenu() {
      const btn = i("mobile-menu-button"),
        menu = i("mobile-menu"),
        overlay = i("mobile-menu-overlay"),
        closeBtn = i("mobile-menu-close");
      ...
      if (closeBtn) r(closeBtn, "click", closeMenu);
      r(overlay, "click", closeMenu);

      f("#mobile-menu a").forEach(link => r(link, "click", closeMenu));

      r(document, "keydown", (e) => {
        if (e.key === "Escape") closeMenu();
      });

      // Fechar menu em mudança de orientação ou resize
    ...
    initScrollAnimations() { let e = (t, s, o = {}) => { let c = new IntersectionObserver(a => { a.forEach(l => { l.isIntersecting && (l.target.classList.add(s), c.unobserve(l.target)) }) }, { threshold: .1, ...o }); f(t).forEach(a => c.observe(a)) }; e(".animate-on-scroll", "is-visible"), e(".stagger-item", "is-visible", { rootMargin: "-50px" }), e(".text-reveal, .text-slide, .text-reveal-letter", "revealed", { threshold: .3 }) } initCounters() { let e = s => 1 - Math.pow(2, -10 * s), t = s => s.toLocaleString("pt-BR"); f(".counter").forEach(s => { new IntersectionObserver(([c]) => { if (!c.isIntersecting) return; let a = +s.dataset.target, l = s.dataset.suffix || "", m = performance.now(), C = 2500, w = M => { let B = M - m, b = Math.min(B / C, 1), P = Math.floor(a * e(b)); s.textContent = t(P) + l, b < 1 && requestAnimationFrame(w) }; requestAnimationFrame(w) }, { threshold: .5 }).observe(s) }) } initFAQ() { r(document, "click", ".faq-question", e => { let t = e.target.closest(".faq-item"), s = t.classList.contains("group-open"); f(".faq-item.group-open").forEach(o => o.classList.remove("group-open")), s || t.classList.add("group-open") }) }
    initScrollToTop() { let e = i("back-to-top-btn"); e && (r(window, "scroll", k(() => { d(e, "show", window.scrollY > 300) }, 100)), r(e, "click", () => { window.scrollTo({ top: 0, behavior: "smooth" }) })) } initParallax() { let e = document.querySelector(".section-background"); e && r(window, "scroll", k(() => { e.style.backgroundPositionY = `${window.scrollY * .5}px` }, 10)) } initForm() {
      ...
      const showToast = (message, isError = false) => {
        ...
        toast.className = `fixed bottom-24 left-1/2 transform -translate-x-1/2 px-6 py-4 rounded-xl shadow-2xl z-50 transition-all duration-500 flex items-center gap-3 backdrop-blur-md ${isError ? 'bg-red-500/90 text-white' : 'bg-green-500/90 text-white'}`;
        toast.innerHTML = `<i data-lucide="${isError ? 'alert-circle' : 'check-circle'}"></i> <span>${message}</span>`;
      ...
      r(e, "submit", async t => {
        ...
        if (!S(o.name) || !VNF(o.name)) {
          showToast("Por favor, insira seu nome completo (nome e sobrenome).", true);
          return;
        }
        if (!L(o.email)) {
        ...
        if (!I(o.whatsapp)) {
          showToast("Por favor, insira um número de WhatsApp válido.", true);
          return;
        }
        if (!S(o.message, 10)) {
          showToast("A mensagem deve ter pelo menos 10 caracteres.", true);
          return;
        }
        ...
        try {
          ...
          // Fallback: Salvar no localStorage para retry posterior
          try {
            const pending = JSON.parse(localStorage.getItem('pendingForms') || '[]');
            pending.push({ ...o, attemptedAt: new Date().toISOString() });
            localStorage.setItem('pendingForms', JSON.stringify(pending));
            showToast("Erro no envio. Mensagem salva localmente. Tentaremos reenviar automaticamente.", true);
          } catch (storageError) {
    ...
    initPhoneMask() { r(document, "input", "#whatsapp", e => { e.target.value = T(e.target.value) }) } initCTATracking() { f('a[href="#contact"]').forEach(e => { r(e, "click", () => { typeof trackEvent == "function" && trackEvent("cta_click", { event_category: "engagement", event_label: e.textContent.trim() }) }) }) } async retryPendingForms() { try { const pending = JSON.parse(localStorage.getItem('pendingForms') || '[]'); if (pending.length === 0) return; const successful = []; for (let i = 0; i < pending.length; i++) { try { const response = await fetch("https://api.web3forms.com/submit", { method: "POST", headers: { "Content-Type": "application/json", "Accept": "application/json" }, body: JSON.stringify(pending[i]) }); if (response.ok) { successful.push(i); } else { break; } } catch (e) { break; } } if (successful.length > 0) { const remaining = pending.filter((_, idx) => !successful.includes(idx)); localStorage.setItem('pendingForms', JSON.stringify(remaining)); console.log(`[Retry] ${successful.length} formulário(s) reenviado(s) com sucesso`); } } catch (e) { console.warn("Erro ao tentar reenviar formulários pendentes:", e); } } refreshIcons() { try { window.lucide?.createIcons?.({ icons: window.lucide?.icons }) } catch (e) { console.warn("Lucide icons initialization failed:", e) } }
  };
  ...
  (function () {
    ...
    function step() {
      ...
      if (!deleting && char > text.length) {
        deleting = true;
        speed = 4000; // Pausa mais longa para permitir a leitura
